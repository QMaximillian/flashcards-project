...
router.patch("/update-flashcard-count", checkJwt, async (req, res) => {
  try {
    ...
    await knex("card_sets").where({ id }).update({ flashcards_count });
...
router.patch("/card-set", checkJwt, async (req, res) => {
  try {
    ...
    await knex("card_sets").where({ id }).update({
      private: isPrivate,
    });
...
router.post("/users-card-set/new", checkJwt, async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      await knex("users_card_sets").insert({
        id: uuidv4(),
        user_id,
        card_set_id,
        ...
        creator_id:
          !creator_id || creator_id === user_id ? user_id : creator_id,
      });
    }
...
router.get("/studied/:username", async (req, res) => {
  try {
    const cardSets = await knex
      .select(
        "users_card_sets.last_studied_at",
